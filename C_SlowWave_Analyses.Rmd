...
**Major changes: Updated plots of change over time to take away points and include CIs.**
...
tapply(SlowWave.correct$MeanCurr, SlowWave.correct$Condition, sd, na.rm=T)
```

Average number of trials in each condition for each subject:
``` {r}
data.frame(iC = nrow(filter(SlowWave.correct, TrialCondition == "Previous incompatible - Current compatible"))/36/9,
           cC = nrow(filter(SlowWave.correct, TrialCondition == "Previous compatible - Current compatible"))/36/9,
           cI = nrow(filter(SlowWave.correct, TrialCondition == "Previous compatible - Current incompatible"))/36/9,
           iI = nrow(filter(SlowWave.correct, TrialCondition == "Previous incompatible - Current incompatible"))/36/9)
...
ggplot(SE.dat, aes(Trial, Point, group = Condition)) +
  ...
  theme_bw() +
  scale_y_continuous(limits =c(-5.5, 2), expand=c(0,0)) +
