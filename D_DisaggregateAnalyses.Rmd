...
author: Curt, 3/08/2017, Hannah, 4/7/2017
output:
  html_document:
    highlight: pygments
    theme: cerulean
...
MeanCent_dat = read.delim("N2prev_FSWprev_MeanCent_EventFixed_AddN2curr.txt")
...
**Added FSW x Current x Previous -> N2**
...
MeanCent_dat$Previous[MeanCent_dat$Cell == "Previous incompatible - Current compatible"] = "Inompatible"
...
**PI-CI**  
``` {r PICI, echo = FALSE}
PICI = lmer(N2curr ~ SlowWaveMean + SlowWaveCent +
...
**PC-CI**  
``` {r PCCI, echo = FALSE}
PCCI = lmer(N2curr ~ SlowWaveMean + SlowWaveCent +
                    (1|Subject), data = filter(MeanCent_dat, Cell == "Previous compatible - Current incompatible"))

summary(PCCI)
```

**Test previous when current is incompatible**  
``` {r testPrev, echo = FALSE}
testPrev = lmer(N2curr ~ SlowWaveMean + SlowWaveCent*Previous +
                    (1|Subject), data = filter(MeanCent_dat, Current == "Incompatible"))
...
#  geom_hline(yintercept=0) +
  ...
  labs(x = "FSW on Trial N - 1 (uV)", y = "RT on Trial N (ms)") +
  ylim(421, 423) +
...
```{r data3}
Dis_N2_Out = lmer(N2curr ~ N2prevMean + N2prevCent + 
                    (1|Subject), data = MeanCent_dat)
```
...
```{r Cdata1}
Dis_N2_Out2 = lmer(N2curr ~ N2prevMean + N2prevCent*Trial.scaled + 
                    (1|Subject), data = MeanCent_dat)
```
...
### 5. N2 on one trial predicting FSW on the same trial

```{r data5}
Dis_SW_Out = lmer(SlowWavePrev ~ N2prevMean + N2prevCent + 
                    (1|Subject), data = MeanCent_dat)
```
...
```{r Ddata5}
Dis_SW_Out2 = lmer(SlowWavePrev ~ N2prevMean + N2prevCent*Trial.scaled + 
                    (1|Subject), data = MeanCent_dat)
```
...
### 6. N2 on one trial predicting RT on the same trial

```{r data6}
Dis_N2_Out = lmer(RT ~ N2currMean + N2currCent + 
                    (1|Subject), data = MeanCent_dat)
```
  
**Random effects:**  
``` {r grand6random, echo = FALSE}
summary(Dis_N2_Out)$varcor
```

**Fixed effects:**
``` {r grand6fixed, echo = FALSE} 
est = as.data.frame(round(summary(Dis_N2_Out)$coefficients, digits = 3))
est
```


#### E. N2 on one trial predicting RT on the same trial, including trial as a predictor
```{r Ddata6}
Dis_N2_Out2 = lmer(RT ~ N2currMean + N2currCent*Trial.scaled + 
                    (1|Subject), data = MeanCent_dat)
```

**Random effects:**  
``` {r Dgrand6random, echo = FALSE}
summary(Dis_N2_Out2)$varcor
```

**Fixed effects:**
``` {r Dgrand6fixed, echo = FALSE} 
est = as.data.frame(round(summary(Dis_N2_Out2)$coefficients, digits = 3))
est
```

#### E2. N2 on one trial predicting RT on the same trial, just for iI trials
```{r Ddata7}
Dis_N2_Out2 = lmer(RT ~ N2currMean + N2currCent + 
                    (1|Subject), data = filter(MeanCent_dat, Cell == "Previous incompatible - Current incompatible"))
```

**Random effects:**  
``` {r Dgrand7random, echo = FALSE}
summary(Dis_N2_Out2)$varcor
```

**Fixed effects:**
``` {r Dgrand7fixed, echo = FALSE} 
est = as.data.frame(round(summary(Dis_N2_Out2)$coefficients, digits = 3))
est
```
